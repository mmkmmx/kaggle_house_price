...
# I tried to use Jeff's code for dummyvars function from caret package (last lecture) but I was getting weird errors and outcomes. If you can figure it out, then go for it. In the mean time, I replaced NAs with 0's.
# I tried replacing NAs with median in some cases noted below, but the assignments didnt work for some reason that I cant figure out. There arent that many NAs so I figure replacing them with 0's is probably not going to hurt anything downstream.  We can switch strategies if you can figure out how to impute the median values. for some variables, 0 actually makes more sense than the median. 
