...
#dtest$ID 2152 isn't setting NA for Exterior1st to none
...
#stripping out Exterior1st from both datasets to see if I can get a kaggle submission in
d1 <- d1 %>% select(-Exterior1st)
dtest <- dtest %>% select(-Exterior1st)
...
# MM  - Lot Area, GrLivArea look like they might benefit from a log transformation.  However when I ran the scatterplot with the logs i'm not sure it added value for lot area:

#the log variable plots:
...
caret_lm <- train(SalePrice ~ ., data = d1, method = "lm")
...
kaggle_submit <- dtest %>% 
  select(Id) %>% 
  mutate(SalePrice = predict(submit_model, newdata = dtest, na.action = na.pass))


#MM - dtest rows with NAs from the predict function - Something weird is going on with MSZoning - sometimes the factors are showing as integers as sometimes they show as they should.  

#submit_NAs <- kaggle_submit %>%
